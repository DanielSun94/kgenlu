...
fin = open(os.path.abspath('../resource/multiwoz/mapping.pair'), 'r')
replacements = []
...
def fixDelex(filename, data, data2, idx, idx_acts):
    ...
    # useless, the replace action is never hit
    try:
...
def createData():
    ...
    fin1 = open(os.path.abspath('../resource/multiwoz/data.json'), 'r')
    data = json.load(fin1)

    fin2 = open(os.path.abspath('../resource/multiwoz/dialogue_acts.json'), 'r')
...
def divideData(data):
    ...
    testListFile = []
    fin = open(os.path.abspath('../resource/multiwoz/testListFile.json'), 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open(os.path.abspath('../resource/multiwoz/valListFile.json'), 'r')
    for line in fin:
        valListFile.append(line[:-1])
    fin.close()

    trainListFile = open(os.path.abspath('../resource/multiwoz/trainListFile.json'), 'w')
