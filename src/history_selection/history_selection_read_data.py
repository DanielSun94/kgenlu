...
import random
from history_selection_config import args, logger, dev_idx_path, test_idx_path, act_data_path, ACT_SLOT_NAME_MAP_DICT, \
...
def dialogue_reorganize_and_normalize(utterance_list, state_dict, act_dict, classify_slot_value_index_dict):
    ...
    for turn_idx in range(0, len(utterance_list)//2):
        ...
        for domain_slot in domain_slot_list:
            ...
            _, referred_slot, turn_utterance_token_label, class_type, classify_value = \
...
def normalize_label(slot, value_label):
    ...
    if slot == 'restaurant-book-day' or slot == 'hotel-book-day' or slot == 'train-day':
        if value_label == 'thur':
    ...
    if slot == 'restaurant-pricerange' or slot == 'hotel-pricerange':
    ...
    if slot == 'hotel-parking':
        if value_label == 'y' or value_label == 'free' or value_label == 'free parking':
            value_label = 'yes'
        if value_label == 'n':
            value_label = 'no'
    if slot == 'hotel-book-people' or slot == 'hotel-book-stay':
        if value_label == 'six':
            value_label = '6'
        if value_label == '3.':
            value_label = '3'
    if slot == 'hotel-internet':
